...
public:
    ...
    private:
        ...
        // inline int GetMaxHeight() const { return kMaxHeight; }
        // 区别时后一种是动态的原子变量 可以在运行时进行调整
...
bool SkipList<Key, Comparator>::Contains(const Key& key) const {
    ...
    if(x != nullptr && Equal(key, x->key)) return true;
    else return false;
}
